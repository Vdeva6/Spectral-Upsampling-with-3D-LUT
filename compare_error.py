...
def load_cmf(path="data/cie-cmf.txt"):
    ...
    cmf = np.array(data)                # shape: [81, 3]
    cmf_wl = np.array(wavelengths)      # shape: [81]
    return cmf, cmf_wl
...
def main():
    ...
    for i in range(len(RGB)):
        RGB_linear = RGB[i]  # No gamma correction before LUT lookup
        coeffs = look_up_coefficients(RGB_linear, lut)
        ...
        alpha = np.max(RGB[i])
        alpha_scaled = smoothstep(smoothstep(alpha))  # Double smoothstep
